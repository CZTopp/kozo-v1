...
import {
  ...
  fetchSingleIndexQuote,
  fetchSingleFredSeries,
...
import {
  ...
  insertRevenueLineItemSchema,
  insertRevenuePeriodSchema,
  insertIncomeStatementLineSchema,
  insertBalanceSheetLineSchema,
  insertCashFlowLineSchema,
  insertDcfValuationSchema,
  insertValuationComparisonSchema,
  insertPortfolioPositionSchema,
  insertPortfolioLotSchema,
  insertMacroIndicatorSchema,
  insertMarketIndexSchema,
  insertPortfolioRedFlagSchema,
  insertScenarioSchema,
  insertAssumptionsSchema,
  insertActualsSchema,
  insertReportSchema,
  insertCryptoProjectSchema,
  insertTokenSupplyScheduleSchema,
  insertTokenIncentiveSchema,
} from '@shared/schema'
import {
  searchCoins,
  searchCoinByContract,
  looksLikeContractAddress,
  getCoinMarketData,
  getMultipleCoinMarketData,
  mapCoinGeckoToProject,
  ...
  estimateBurnFromSupply,
} from './crypto-data'
import { getOnChainTokenData } from './thirdweb_auth/thirdweb-data'
import {
  getTokenEmissions,
  getBatchTokenEmissions,
  TOKEN_CATEGORIES,
  getTokenCategory,
...
export async function registerRoutes(server: Server, app: Express) {
  ...
  app.patch('/api/models/:id', async (req: Request<Params>, res: Response) => {
    ...
    const numericFields = [
      'growthDecayRate',
      'targetNetMargin',
      'scenarioBullMultiplier',
      'scenarioBaseMultiplier',
      'scenarioBearMultiplier',
      'sharesOutstanding',
      'startYear',
      'endYear',
    ] as const
    ...
    if (
      body.displayUnit &&
      !['ones', 'thousands', 'millions', 'billions', 'trillions'].includes(
        body.displayUnit,
      )
    ) {
      return res.status(400).json({
        message:
          'displayUnit must be ones, thousands, millions, billions, or trillions',
      })
    }
    if (body.modelMode && !['ipo', 'invest'].includes(body.modelMode)) {
      return res
        .status(400)
        .json({ message: "modelMode must be 'ipo' or 'invest'" })
    }
  ...
  app.patch(
    ...
    async (req: Request<Params>, res: Response) => {
      ...
      if (!startYear || !endYear) {
        return res
          .status(400)
          .json({ message: 'startYear and endYear are required' })
      }
      ...
      for (const [oldYear, newYear] of Object.entries(yearMapping)) {
        ...
        if (oldYearNum !== newYearNum) {
          const periodsToUpdate = allPeriods.filter(
            (p) => p.year === oldYearNum,
          )
          for (const p of periodsToUpdate) {
            await db
              .update(revenuePeriods)
              .set({ year: newYearNum })
      ...
      const model = await storage.updateModel(req.params.id, userId, {
        startYear,
        endYear,
      })
      if (!model) return res.status(404).json({ message: 'Model not found' })
  ...
  app.get(
    '/api/models/:modelId/revenue-line-items',
    async (req: Request<Params>, res: Response) => {
  ...
  app.post(
    '/api/models/:modelId/revenue-line-items-with-periods',
    async (req: Request<Params>, res: Response) => {
      const userId = (req as any).user?.claims?.sub as string
      if (!userId) return res.status(401).json({ message: 'Unauthorized' })
      const { name, sortOrder } = req.body
      ...
      const lineItem = await storage.createRevenueLineItem({
        modelId,
        name: name.trim(),
  ...
  app.get(
    '/api/models/:modelId/revenue-periods',
    async (req: Request<Params>, res: Response) => {
  ...
  app.get(
    '/api/models/:modelId/income-statement',
    async (req: Request<Params>, res: Response) => {
  ...
  app.get(
    '/api/models/:modelId/balance-sheet',
    async (req: Request<Params>, res: Response) => {
  ...
  app.patch(
    '/api/models/:modelId/balance-sheet/:year',
    async (req: Request<Params>, res: Response) => {
      const { modelId, year } = req.params
      const yearNum = parseInt(year)
      const data = { ...req.body }

      const existing = (await storage.getBalanceSheetLines(modelId)).find(
        (l) => l.year === yearNum,
      )
      ...
      for (const k of numKeys) {
        merged[k] =
          data[k] !== undefined ? Number(data[k]) : (existing as any)?.[k] || 0
      }

      const totalCA =
        merged.cash +
        merged.shortTermInvestments +
        merged.accountsReceivable +
        merged.inventory
      const totalLTA =
        merged.equipment - merged.depreciationAccum + merged.capex
      ...
      const line = await storage.updateBalanceSheetLineByYear(
        modelId,
        yearNum,
        data,
      )
  ...
  app.patch(
    '/api/models/:modelId/cash-flow/:year',
    async (req: Request<Params>, res: Response) => {
      const { modelId, year } = req.params
      const line = await storage.updateCashFlowLineByYear(
        modelId,
        parseInt(year),
        req.body,
      )
  ...
  app.patch(
    '/api/models/:modelId/income-statement/:year',
    async (req: Request<Params>, res: Response) => {
      const { modelId, year } = req.params
      const line = await storage.updateIncomeStatementLineByYear(
        modelId,
        parseInt(year),
        req.body,
      )
  ...
  app.get(
    '/api/models/:modelId/valuation-comparison',
    async (req: Request<Params>, res: Response) => {
    ...
    async (req: Request<Params>, res: Response) => {
      ...
      if (userId) {
        ...
        if (!limit.allowed)
          return res
            .status(403)
            .json({ message: limit.reason, requiredPlan: limit.requiredPlan })
      }
  ...
  app.post('/api/portfolio', async (req: Request<Params>, res: Response) => {
    ...
    if (!parsed.success)
      return res.status(400).json({ message: parsed.error.message })
    const pos = await storage.createPortfolioPosition({
      ...parsed.data,
      userId,
    })
  ...
  app.patch(
    ...
    async (req: Request<Params>, res: Response) => {
      const userId = (req as any).user?.claims?.sub as string
      if (!userId) return res.status(401).json({ message: 'Unauthorized' })
      const pos = await storage.updatePortfolioPosition(
        req.params.id,
        userId,
        req.body,
      )
  ...
  app.get(
    '/api/portfolio/:positionId/lots',
    async (req: Request<Params>, res: Response) => {
      const lots = await storage.getPortfolioLots(
  ...
  app.post(
    ...
    async (req: Request<Params>, res: Response) => {
      ...
      const position = await storage.recomputePositionFromLots(
        req.params.positionId as string,
      )
  ...
  app.patch(
    '/api/portfolio/lots/:lotId',
    async (req: Request<Params>, res: Response) => {
      const lot = await storage.updatePortfolioLot(
  ...
  app.post('/api/refresh-market-data', async (req: Request, res: Response) => {
    ...
    if (!userId) return res.status(401).json({ message: 'Unauthorized' })
    try {
      const results: { indices: number; macro: number; errors: string[] } = {
        indices: 0,
        macro: 0,
        errors: [],
      }
      ...
      if (fredApiKey) {
        ...
        results.errors.push(
          'FRED_API_KEY not set -- macro indicators not updated. Get a free key at fred.stlouisfed.org.',
        )
      }
    ...
    async (req: Request, res: Response) => {
      ...
      try {
        ...
        for (const p of positions) {
          const yahooTicker = p.isCrypto
            ? `${p.ticker.toUpperCase()}-USD`
            : p.ticker
        ...
        for (const quote of results) {
          ...
          const position = positions.find(
            (p) => p.ticker.toUpperCase() === originalTicker,
          )
          if (position) {
            const ma50 = quote.ma50
            const ma200 = quote.ma200
            const goldenCross = ma50 > ma200 && ma50 > 0 && ma200 > 0
            const changeFromMa50 =
              quote.currentPrice > 0 && ma50 > 0
                ? (quote.currentPrice - ma50) / ma50
                : 0
            const changeFromMa200 =
              quote.currentPrice > 0 && ma200 > 0
                ? (quote.currentPrice - ma200) / ma200
                : 0
            const positionValue =
              (position.sharesHeld || 0) * quote.currentPrice
            const costBasis =
              (position.sharesHeld || 0) * (position.purchasePrice || 0)
            const gainLossDollar = positionValue - costBasis
            const gainLossPercent =
              costBasis > 0 ? gainLossDollar / costBasis : 0
  ...
  app.post(
    ...
    async (req: Request<Params>, res: Response) => {
      ...
      try {
        ...
        if (!quote) {
          return res.status(400).json({
            message: `Could not find data for symbol "${symbol}". Make sure it is a valid Yahoo Finance symbol (e.g., ^GSPC, ^FTSE).`,
          })
        }
  ...
  app.post(
    '/api/macro-indicators/add-custom',
    async (req: Request<Params>, res: Response) => {
      const userId = (req as any).user?.claims?.sub as string
      if (!userId) return res.status(401).json({ message: 'Unauthorized' })
      try {
        ...
        if (!fredApiKey) {
          return res.status(400).json({
            message:
              'FRED_API_KEY not set. Add a free key from fred.stlouisfed.org.',
          })
        }
        const result = await fetchSingleFredSeries(
          seriesId.trim().toUpperCase(),
          fredApiKey,
        )
        if (!result) {
          return res.status(400).json({
            message: `Could not find data for FRED series "${seriesId}". Make sure it is a valid FRED series ID (e.g., DGS10, UNRATE).`,
          })
        }
  ...
  app.get(
    '/api/models/:modelId/assumptions',
    async (req: Request<Params>, res: Response) => {
  ...
  app.post(
    '/api/models/:modelId/recalculate',
    async (req: Request<Params>, res: Response) => {
      try {
  ...
  app.post(
    '/api/models/:modelId/forecast-forward',
    async (req: Request<Params>, res: Response) => {
      try {
  ...
  app.patch(
    ...
    async (req: Request<Params>, res: Response) => {
      ...
      if (base) {
        ...
        const created = await storage.createAssumptions({
  ...
  app.get(
    '/api/models/:modelId/yahoo-fundamentals',
    async (req: Request<Params>, res: Response) => {
      try {
        const userId = (req as any).user?.claims?.sub as string
        if (!userId) return res.status(401).json({ message: 'Unauthorized' })
        ...
        if (!model.ticker)
          return res.status(400).json({
            message:
              'No ticker set for this company. Set a ticker in Edit Company first.',
          })

        const data = await fetchCompanyFundamentals(model.ticker)
        if (!data)
          return res.status(404).json({
            message: `Could not fetch data for ticker ${model.ticker}`,
          })
        ...
        res
          .status(500)
          .json({ message: err.message || 'Failed to fetch Yahoo data' })
      }
    },
  )

  app.post(
    '/api/models/:modelId/sync-yahoo',
    async (req: Request<Params>, res: Response) => {
      try {
        const userId = (req as any).user?.claims?.sub as string
        if (!userId) return res.status(401).json({ message: 'Unauthorized' })
        ...
        if (!model.ticker)
          return res
            .status(400)
            .json({ message: 'No ticker set for this company.' })
        ...
        if (data.sharesOutstanding > 0)
          updates.sharesOutstanding = data.sharesOutstanding
        ...
        if (dcf) {
          ...
          if (data.currentPrice > 0)
            dcfUpdates.currentSharePrice = data.currentPrice
        ...
        if (valComp && data.currentPrice > 0) {
          await storage.upsertValuationComparison({
            ...valComp,
            currentSharePrice: data.currentPrice,
          } as any)
        }
  ...
  app.post('/api/parse-edgar', async (req: Request<Params>, res: Response) => {
    try {
      ...
      res
        .status(500)
        .json({ message: err.message || 'Failed to parse SEC EDGAR filing' })
    }
  })

  app.get(
    '/api/sec/search/:ticker',
    async (req: Request<Params>, res: Response) => {
      try {
        const result = await searchCompanyByTicker(req.params.ticker)
        if (!result)
          return res.status(404).json({
            message: `Ticker "${req.params.ticker}" not found on SEC EDGAR`,
          })
        res.json(result)
      } catch (err: any) {
  ...
  app.get(
    ...
    async (req: Request<Params>, res: Response) => {
      try {
        ...
        res
          .status(500)
          .json({ message: err.message || 'Failed to fetch filings' })
      }
    },
  )

  app.post(
    '/api/sec/parse-all-statements',
    async (req: Request<Params>, res: Response) => {
      try {
        ...
        res
          .status(500)
          .json({ message: err.message || 'Failed to parse SEC filing' })
      }
    },
  )

  app.post(
    '/api/models/:modelId/import-sec',
    async (req: Request<Params>, res: Response) => {
      try {
        const userId = (req as any).user?.claims?.sub as string
        if (!userId) return res.status(401).json({ message: 'Unauthorized' })
        ...
        if (!secLimit.allowed)
          return res.status(403).json({
            message: secLimit.reason,
            requiredPlan: secLimit.requiredPlan,
          })
        const modelId = req.params.modelId
        const model = await storage.getModel(modelId, userId)
        if (!model) return res.status(404).json({ message: 'Model not found' })
        ...
        if (newStartYear !== model.startYear || newEndYear !== model.endYear) {
          await storage.updateModel(modelId, userId, {
        ...
        let totalRevLineItem = existingLineItems.find(
          (li) => li.name === 'Total Revenue',
        )
        if (!totalRevLineItem) {
        ...
        for (const year of importedYears) {
          ...
          if (isData) {
            ...
            const operatingIncome =
              grossProfit - sgaExpense - rdExpense - depreciation
            ...
            await storage.upsertIncomeStatementLine({
              ...
              effectiveTaxRate:
                preTaxIncome > 0 ? taxExpense / preTaxIncome : 0,
              interestExpense,
            } as any)
          }
        }
        ...
        for (const year of importedYears) {
          ...
          if (bsData) {
            await storage.upsertBalanceSheetLine({
              ...
              capex: 0,
              totalLongTermAssets:
                (bsData.totalAssets || 0) - (bsData.totalCurrentAssets || 0),
        ...
        for (const year of importedYears) {
          ...
          if (cfData) {
            await storage.upsertCashFlowLine({
              ...
              freeCashFlow:
                (cfData.operatingCashFlow || 0) - Math.abs(cfData.capex || 0),
            })
          }
        }
        ...
        res.json({
          ...
          importedYears,
          yearRangeExpanded:
            newStartYear !== model.startYear || newEndYear !== model.endYear,
          newStartYear,
          newEndYear,
          statements: {
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const upcomingByProject: Record<
          string,
          {
            projectId: string
            projectName: string
  ...
  app.post(
    ...
    async (req: Request<Params>, res: Response) => {
      ...
      try {
        ...
        if (!coingeckoId)
          return res.status(400).json({ message: 'coingeckoId is required' })
        let coinData
        try {
          ...
          if (fetchErr.message === 'RATE_LIMITED') {
            return res.status(429).json({
              message:
                'CoinGecko rate limit reached. Please wait a few seconds and try again.',
            })
          }
          return res.status(502).json({
            message: `Could not reach CoinGecko API. Please try again shortly.`,
          })
        }
        if (!coinData)
          return res
            .status(400)
            .json({ message: `Could not find data for "${coingeckoId}"` })
        const projectData = mapCoinGeckoToProject(coinData)
        const project = await storage.createCryptoProject({
          ...projectData,
          userId,
        })
  ...
  app.patch(
    ...
    async (req: Request<Params>, res: Response) => {
      ...
      try {
        ...
        for (const key of Object.keys(req.body)) {
          if (allowedFields.includes(key)) {
            ...
            if (numericFields.includes(key)) {
              ...
              filtered[key] =
                val != null ? Math.max(1, Math.min(20, Number(val) || 5)) : 5
        ...
        const project = await storage.updateCryptoProject(
          req.params.id,
          userId,
          filtered,
        )
  ...
  app.post(
    '/api/crypto/projects/:id/parse-pdf',
    async (req: Request<Params>, res: Response) => {
      const userId = (req as any).user?.claims?.sub as string
      if (!userId) return res.status(401).json({ message: 'Unauthorized' })
      ...
      if (!pdfLimit.allowed)
        return res.status(403).json({
          message: pdfLimit.reason,
          requiredPlan: pdfLimit.requiredPlan,
        })
      try {
        const project = await storage.getCryptoProject(req.params.id, userId)
        if (!project)
        ...
        await storage.updateCryptoProject(req.params.id, userId, {
          whitepaper: truncated,
        })
  ...
  app.get(
    '/api/crypto/projects/:id/supply-schedules',
    async (req: Request<Params>, res: Response) => {
  ...
  app.patch(
    '/api/crypto/supply-schedules/:id',
    async (req: Request<Params>, res: Response) => {
      const schedule = await storage.updateTokenSupplySchedule(
        req.params.id,
        req.body,
      )
      res.json(schedule)
  ...
  app.get(
    '/api/crypto/projects/:id/incentives',
    async (req: Request<Params>, res: Response) => {
  ...
  app.patch(
    '/api/crypto/incentives/:id',
    async (req: Request<Params>, res: Response) => {
      const incentive = await storage.updateTokenIncentive(
        req.params.id,
        req.body,
      )
      res.json(incentive)
  ...
  app.get(
    '/api/crypto/incentive-templates/:key',
    async (req: Request<Params>, res: Response) => {
  ...
  app.get(
    '/api/crypto/projects/:id/protocol-metrics',
    async (req: Request<Params>, res: Response) => {
  ...
  app.post(
    '/api/crypto/projects/:id/fetch-defi-data',
    async (req: Request<Params>, res: Response) => {
      try {
        const userId = (req as any).user?.claims?.sub as string
        if (!userId) return res.status(401).json({ message: 'Unauthorized' })
        ...
        if (!project)
          return res.status(404).json({ message: 'Project not found' })
        const slug =
          project.defiLlamaId || project.name.toLowerCase().replace(/\s+/g, '-')
        ...
        const defiSummary = {
          ...
          latestTvl:
            latestMetrics.length > 0
              ? latestMetrics[latestMetrics.length - 1].tvl
              : 0,
          latestFees:
            latestMetrics.length > 0
              ? latestMetrics[latestMetrics.length - 1].dailyFees
              : 0,
          latestRevenue:
            latestMetrics.length > 0
              ? latestMetrics[latestMetrics.length - 1].dailyRevenue
              : 0,
        }
  ...
  app.get(
    '/api/crypto/projects/:id/contract-address',
    async (req: Request<Params>, res: Response) => {
      try {
        const userId = (req as any).user?.claims?.sub as string
        if (!userId) return res.status(401).json({ message: 'Unauthorized' })
  ...
  app.get(
    '/api/crypto/projects/:id/cached-data',
    async (req: Request<Params>, res: Response) => {
      try {
        const userId = (req as any).user?.claims?.sub as string
        if (!userId) return res.status(401).json({ message: 'Unauthorized' })
        ...
        const onchainAge = project.onchainDataFetchedAt
          ? now - new Date(project.onchainDataFetchedAt).getTime()
          : null
        const defiAge = project.defiDataFetchedAt
          ? now - new Date(project.defiDataFetchedAt).getTime()
          : null
  ...
  app.post(
    '/api/crypto/projects/:id/onchain-data',
    async (req: Request<Params>, res: Response) => {
      try {
        const userId = (req as any).user?.claims?.sub as string
        if (!userId) return res.status(401).json({ message: 'Unauthorized' })
        ...
        if (isEvmChain) {
          ...
          const burnEstimate = estimateBurnFromSupply(
            project.totalSupply,
            project.maxSupply,
          ...
          const noteLines = [
            `On-chain queries not available for ${chainLabel}.`,
          ]
          if (burnEstimate.source === 'max_supply_delta') {
            noteLines.push(
              `Burn data derived from CoinGecko: maxSupply (${formatNum(project.maxSupply)}) vs totalSupply (${formatNum(project.totalSupply)}).`,
            )
          } else if (burnEstimate.source === 'supply_gap') {
            noteLines.push(
              `Supply gap detected (${formatNum((project.totalSupply || 0) - (project.circulatingSupply || 0))} tokens locked/vesting). No confirmed burn program from supply data — use manual entry for known burns/buybacks.`,
            )
          } else {
            noteLines.push(
              'No burn signal from supply data. Use manual entry if this token has a burn/buyback program.',
            )
          }
          const result = {
            burns: {
  ...
  app.get(
    '/api/crypto/projects/:id/revenue-forecasts',
    async (req: Request<Params>, res: Response) => {
      const userId = (req as any).user?.claims?.sub as string
      if (!userId) return res.status(401).json({ message: 'Unauthorized' })
      const project = await storage.getCryptoProject(req.params.id, userId)
  ...
  app.post(
    '/api/crypto/projects/:id/revenue-forecasts/seed',
    async (req: Request<Params>, res: Response) => {
      try {
        const userId = (req as any).user?.claims?.sub as string
        if (!userId) return res.status(401).json({ message: 'Unauthorized' })
        ...
        for (const m of metrics) {
          ...
          if (!isNaN(year)) {
            ...
            yearlyRevenue[year] =
              (yearlyRevenue[year] || 0) + (m.dailyRevenue || 0)
          }
        }
        ...
        const totalEmissionPercent = incentives.reduce(
          (sum, inc) => sum + (inc.allocationPercent || 0),
          0,
        )
        ...
        for (let i = 1; i <= projYears; i++) {
          ...
          const prevFees =
            i === 1
              ? lastFees
              : forecasts[forecasts.length - 1]?.projectedFees || lastFees
          const fees = prevFees * (1 + growth)
          const revenue = fees * takeRate
          const emission =
            revenue * (totalEmissionPercent / 100) * Math.pow(0.8, i - 1)
          forecasts.push({
            projectId: project.id,
            year: yr,
  ...
  app.get(
    '/api/crypto/projects/:id/token-flows',
    async (req: Request<Params>, res: Response) => {
      const userId = (req as any).user?.claims?.sub as string
      if (!userId) return res.status(401).json({ message: 'Unauthorized' })
      const project = await storage.getCryptoProject(req.params.id, userId)
  ...
  app.post(
    '/api/crypto/projects/:id/token-flows/seed',
    async (req: Request<Params>, res: Response) => {
      try {
        const userId = (req as any).user?.claims?.sub as string
        if (!userId) return res.status(401).json({ message: 'Unauthorized' })
        ...
        const totalScheduleTokens = schedules.reduce(
          (sum, s) => sum + (s.amount || 0),
          0,
        )
        const avgUnlockPerPeriod =
          totalScheduleTokens > 0 ? totalScheduleTokens / periods : 0
        ...
        for (let i = 0; i < periods; i++) {
          ...
          const minting =
            i < 6 ? (maxSupply - totalSupply) * 0.01 * Math.pow(0.85, i) : 0
  ...
  app.get(
    '/api/crypto/projects/:id/allocations',
    async (req: Request, res: Response) => {
      const userId = (req as any).user?.claims?.sub as string
      if (!userId) return res.status(401).json({ message: 'Unauthorized' })
      const project = await storage.getCryptoProject(
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      ...
      if (!project)
        return res.status(404).json({ message: 'Project not found' })
      const allocation = await storage.createTokenAllocation({
        ...req.body,
        projectId: project.id,
      })
      res.json(allocation)
  ...
  app.post(
    '/api/crypto/projects/:id/allocations/seed',
    async (req: Request, res: Response) => {
      const userId = (req as any).user?.claims?.sub as string
      if (!userId) return res.status(401).json({ message: 'Unauthorized' })
      const project = await storage.getCryptoProject(
      ...
      if (existing.length > 0)
        return res.status(400).json({
          message: 'Allocations already exist. Clear them first to re-seed.',
        })

      const {
        lookupCuratedAllocations,
        mapCuratedToAllocations,
        researchAllocationsWithAI,
        mapAIToAllocations,
      } = await import('./crypto-data')
      let allocationsToCreate: Record<string, unknown>[] = []
      let source = 'fallback'
      const hasDataSources =
        Array.isArray((project as any).dataSources) &&
        (project as any).dataSources.length > 0
      const coingeckoId = project.coingeckoId || ''

      if (coingeckoId && !hasDataSources) {
        const cached = await storage.getAiResearchCache(
          coingeckoId,
          'allocations',
        )
        if (cached) {
          ...
          allocationsToCreate = mapAIToAllocations(
            cachedData,
            project.id,
            totalSupply,
          )
      ...
      if (allocationsToCreate.length === 0 && hasDataSources) {
        try {
          ...
          const totalSupply = project.totalSupply || project.maxSupply || null
          const aiResult = await researchAllocationsWithAI(
            tokenName,
            tokenSymbol,
            totalSupply,
            (project as any).dataSources,
          )
      ...
      if (allocationsToCreate.length === 0 && !hasDataSources) {
        ...
        for (const s of slugsToTry) {
          ...
          if (curatedData) {
            allocationsToCreate = mapCuratedToAllocations(
              curatedData,
              project.id,
            )
            source = 'curated'
      ...
      if (allocationsToCreate.length === 0 && !hasDataSources) {
        try {
          ...
          if (aiResult && aiResult.allocations.length > 0) {
            ...
            source = `ai-researched:${aiResult.confidence}`
            if (coingeckoId) {
              await storage.setAiResearchCache({
                coingeckoId,
                researchType: 'allocations',
                data: aiResult,
          ...
          console.error(
            'AI allocation research failed, falling back to template:',
            err,
          )
        }
      }
        ...
        allocationsToCreate = [
          {
            ...
            standardGroup: 'team',
            percentage: 20,
            amount: totalSupply ? totalSupply * 0.2 : null,
            vestingMonths: 48,
            cliffMonths: 12,
            tgePercent: 0,
            vestingType: 'linear',
            dataSource: 'Industry Average',
            description: 'Core contributors, advisors, partners',
            notes: null,
            sortOrder: 0,
          },
          {
            ...
            percentage: 16,
            amount: totalSupply ? totalSupply * 0.16 : null,
            vestingMonths: 36,
            cliffMonths: 6,
            tgePercent: 0,
            vestingType: 'linear',
            dataSource: 'Industry Average',
            description: 'Seed, Series A/B, strategic rounds',
            notes: null,
            sortOrder: 1,
          },
          {
            ...
            percentage: 5,
            amount: totalSupply ? totalSupply * 0.05 : null,
            vestingMonths: 0,
            cliffMonths: 0,
            tgePercent: 100,
            vestingType: 'immediate',
            dataSource: 'Industry Average',
            description: 'ICO, IEO, IDO, public sale',
            notes: null,
            sortOrder: 2,
          },
          {
            ...
            percentage: 27,
            amount: totalSupply ? totalSupply * 0.27 : null,
            vestingMonths: 30,
            cliffMonths: 0,
            tgePercent: 10,
            vestingType: 'linear',
            dataSource: 'Industry Average',
            description: 'Foundation funds, ecosystem, future initiatives',
            notes: null,
            sortOrder: 3,
          },
          {
            ...
            percentage: 32,
            amount: totalSupply ? totalSupply * 0.32 : null,
            vestingMonths: 30,
            cliffMonths: 0,
            tgePercent: 5,
            vestingType: 'linear',
            dataSource: 'Industry Average',
            description: 'Airdrops, staking rewards, liquidity mining, grants',
            notes: null,
            sortOrder: 4,
          },
        ]
  ...
  app.delete(
    '/api/crypto/projects/:id/allocations/clear',
    async (req: Request, res: Response) => {
      const userId = (req as any).user?.claims?.sub as string
      if (!userId) return res.status(401).json({ message: 'Unauthorized' })
      const project = await storage.getCryptoProject(
  ...
  app.patch(
    '/api/crypto/allocations/:id',
    async (req: Request, res: Response) => {
      const allocation = await storage.updateTokenAllocation(
        req.params.id as string,
        req.body,
      )
      res.json(allocation)
  ...
  app.get(
    '/api/crypto/projects/:id/fundraising',
    async (req: Request, res: Response) => {
      const userId = (req as any).user?.claims?.sub as string
      if (!userId) return res.status(401).json({ message: 'Unauthorized' })
      const project = await storage.getCryptoProject(
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      ...
      if (!project)
        return res.status(404).json({ message: 'Project not found' })
      const round = await storage.createFundraisingRound({
        ...req.body,
        projectId: project.id,
      })
      res.json(round)
  ...
  app.patch(
    '/api/crypto/fundraising/:id',
    async (req: Request, res: Response) => {
      const round = await storage.updateFundraisingRound(
        req.params.id as string,
        req.body,
      )
      res.json(round)
  ...
  app.post(
    '/api/crypto/projects/:id/fundraising/seed',
    async (req: Request, res: Response) => {
      const userId = (req as any).user?.claims?.sub as string
      if (!userId) return res.status(401).json({ message: 'Unauthorized' })
      const project = await storage.getCryptoProject(
      ...
      if (existing.length > 0)
        return res.status(400).json({
          message:
            'Fundraising rounds already exist. Clear them first to re-seed.',
        })

      const { researchFundraisingWithAI, mapAIToFundraisingRounds } =
        await import('./crypto-data')
      const tokenName = project.name || ''
      const tokenSymbol = project.symbol || ''
      const coingeckoId = project.coingeckoId || ''
      ...
      try {
        if (coingeckoId && !hasDataSources) {
          ...
          if (cached) {
            const cachedData = cached.data as any
            const roundsToCreate = mapAIToFundraisingRounds(
              cachedData,
              project.id,
            )
            const results = []
        ...
        if (!aiResult || aiResult.rounds.length === 0) {
          return res.json({
            source: 'none',
            rounds: [],
            notes: 'No fundraising data found for this token.',
          })
        }
        if (coingeckoId && !hasDataSources) {
          await storage.setAiResearchCache({
            coingeckoId,
            researchType: 'fundraising',
            data: aiResult,
            confidence: aiResult.confidence,
            notes: aiResult.notes,
          })
  ...
  app.delete(
    '/api/crypto/projects/:id/fundraising/clear',
    async (req: Request, res: Response) => {
      const userId = (req as any).user?.claims?.sub as string
      if (!userId) return res.status(401).json({ message: 'Unauthorized' })
      const project = await storage.getCryptoProject(
  ...
  app.post(
    '/api/crypto/projects/:id/supply-schedule/seed',
    async (req: Request, res: Response) => {
      const userId = (req as any).user?.claims?.sub as string
      if (!userId) return res.status(401).json({ message: 'Unauthorized' })
      const project = await storage.getCryptoProject(
      ...
      if (existing.length > 0)
        return res.status(400).json({
          message: 'Supply schedule already exists. Clear it first to re-seed.',
        })

      const { researchSupplyScheduleWithAI, mapAIToSupplySchedule } =
        await import('./crypto-data')
      const tokenName = project.name || ''
      const tokenSymbol = project.symbol || ''
      const totalSupply = project.totalSupply || project.maxSupply || null
      ...
      try {
        if (coingeckoId && !hasDataSources) {
          ...
          if (cached) {
            const cachedData = cached.data as any
            const schedulesToCreate = mapAIToSupplySchedule(
              cachedData,
              project.id,
            )
            const results = []
            for (const schedule of schedulesToCreate) {
              const created = await storage.createTokenSupplySchedule(
                schedule as any,
              )
              results.push(created)
            }
        ...
        if (!aiResult || aiResult.events.length === 0) {
          return res.json({
            source: 'none',
            schedules: [],
            notes: 'No supply schedule data found for this token.',
          })
        }
        if (coingeckoId && !hasDataSources) {
          await storage.setAiResearchCache({
            coingeckoId,
            researchType: 'supply_schedule',
            data: aiResult,
            confidence: aiResult.confidence,
            notes: aiResult.notes,
          })
        ...
        res
          .status(500)
          .json({ message: 'Failed to research supply schedule data' })
      }
    },
  )

  app.delete(
    '/api/crypto/projects/:id/supply-schedule/clear',
    async (req: Request, res: Response) => {
      const userId = (req as any).user?.claims?.sub as string
      if (!userId) return res.status(401).json({ message: 'Unauthorized' })
      const project = await storage.getCryptoProject(
  ...
  app.post('/api/copilot', async (req: Request, res: Response) => {
    try {
      ...
      if (!aiLimit.allowed)
        return res
          .status(403)
          .json({ message: aiLimit.reason, requiredPlan: aiLimit.requiredPlan })
      const {
        modelId,
        cryptoProjectId,
        message,
      ...
      if (
        !modelId &&
        !cryptoProjectId &&
        contextType !== 'crypto-dashboard' &&
        contextType !== 'general'
      ) {
        return res.status(400).json({
          message: 'modelId, cryptoProjectId, or context type is required',
        })
      }
      ...
      try {
        ...
        res.write(
          `data: ${JSON.stringify({ error: streamErr.message || 'Stream error' })}\n\n`,
        )
      }
      if (streamSuccess) {
    ...
    async (_req: Request, res: Response) => {
      ...
      const enriched = allUsers.map((u) => {
        ...
        return {
          ...
          positionCount:
            positionsPerUser.find((p) => p.userId === u.id)?.value ?? 0,
          cryptoProjectCount:
            cryptoPerUser.find((c) => c.userId === u.id)?.value ?? 0,
  ...
  app.patch(
    ...
    async (req: Request<Params>, res: Response) => {
      ...
      if (typeof aiCallsUsed === 'number' && aiCallsUsed >= 0)
        updates.aiCallsUsed = aiCallsUsed
      if (typeof pdfParsesUsed === 'number' && pdfParsesUsed >= 0)
        updates.pdfParsesUsed = pdfParsesUsed
      const [updated] = await db
  ...
  app.patch(
    ...
    async (req: Request<Params>, res: Response) => {
      ...
      if (!['free', 'pro', 'enterprise'].includes(plan)) {
        return res
          .status(400)
          .json({ message: 'plan must be free, pro, or enterprise' })
      }
      const targetUserId = req.params.id
      const [existingSub] = await db
        .select()
  ...
  app.get(
    '/api/crypto/emissions/:coingeckoId',
    async (req: Request<Params>, res: Response) => {
      try {
        const { coingeckoId } = req.params
        if (!coingeckoId)
          return res.status(400).json({ message: 'Missing coingeckoId' })

        const result = await getTokenEmissions(coingeckoId)
        if (!result)
          return res.status(404).json({
            message: 'Could not retrieve emission data for this token',
          })
        ...
        if (err.message === 'RATE_LIMITED') {
          return res.status(429).json({
            message: 'Rate limited by CoinGecko. Please try again in a moment.',
          })
        }
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (!ids || !Array.isArray(ids) || ids.length === 0) {
          return res
            .status(400)
            .json({ message: 'Missing or invalid ids array' })
        }
        if (ids.length > 30) {
          return res
            .status(400)
            .json({ message: 'Maximum 30 tokens per batch request' })
        }
  ...
  app.get(
    '/api/crypto/emissions-categories',
  ...
  app.post('/api/stripe/cancel', async (req: Request, res: Response) => {
    try {
      const userId = (req as any).user?.claims?.sub as string
      if (!userId) return res.status(401).json({ message: 'Not authenticated' })
      ...
      res
        .status(500)
        .json({ message: err.message || 'Failed to cancel subscription' })
    }
  })
